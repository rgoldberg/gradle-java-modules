        ...
        AutoGeneratableCascading by DefaultAutoGeneratableCascading(this) {
            ...
            get() =
                super.value.appendAutoGeneratedIterator(autoGenerate.isEnabled && javaCompile.isTestInput) {
                    ...
                    javaCompile.project.tasks.withType(Test::class.java).stream()
                    ...
                    if (moduleNameIbyModuleInfoJavaPath.size < 2) {
                        ...
                        outputClassesDirs.setFrom(
                            *stream(outputClassesDirs)
                            .flatMap {dirFile -> moduleNameIbyModuleInfoJavaPath.values.stream().map {moduleName -> File(dirFile, moduleName)}}
                            .toArray()
                        )
        ...
        AutoGeneratableCascading by DefaultAutoGeneratableCascading(this) {
            ...
            get() {
                ...
                val valueSet by lazy {
                    project.tasks.withType(Test::class.java).stream()
                    .flatMap {it.moduleNameIset.stream()}
                    .collect(toCollection(::createCollection))
                }
                ...
                return valueMutable.appendAutoGeneratedMapFromPairIterator(
                    ...
                    //TODO: ensure works
                    //TODO: only use Test for javaCompile instead of all Tests
                    moduleNameIset.stream().map {it to valueSet}.iterator()
                }
            }
        }
    }
